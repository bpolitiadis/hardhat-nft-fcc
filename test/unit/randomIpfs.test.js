...
!developmentChains.includes(network.name)
    ...
    : describe("Random IPFS NFT Unit Tests", function () {
          ...
          describe("getBreedFromModdedRng", () => {
              it("should return legendary Aura if moddedRng < 10", async function () {
                  const expectedValue = await randomIpfsNft.getBreedFromModdedRng(7);
                  assert.equal(0, expectedValue);
              });
              it("should return rare Aura if moddedRng is between 10 - 39", async function () {
                  const expectedValue = await randomIpfsNft.getBreedFromModdedRng(21);
                  assert.equal(1, expectedValue);
              });
              it("should return common Aura if moddedRng is between 40 - 99", async function () {
